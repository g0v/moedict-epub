...
def select_with_refs(font, unicode, newfont, pe = None, nam = None):
    try:
        newfont.selection.select(('unicode', 'more'), unicode)
    except:
        print 'Failed to select u+%04x character' % unicode
        return
...
def main(argv):
    optlist, args = getopt.gnu_getopt(argv, '', ['string=', 'stringfile=', 'strip_names',
                                                 'simplify', 'new', 'script',
    ...
    elif '--stringfile' in opts:
        f = open(opts['--stringfile'], 'r')
        subset = list(set(map(ord, f.read().decode("UTF-8"))))
        f.close()
    else:
